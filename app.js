...
setTimeout(function() {
            ...
            const biometricManager = Telegram.WebApp.BiometricManager;
            if (!biometricManager.isInited) {
                return DemoApp.showAlert('Attendi qualche secondo...');
            }

            el.nextElementSibling.innerHTML = '';
            el.nextElementSibling.classList.remove('ok', 'err')

            biometricManager.authenticate({reason: 'Autenticati prima di accedere al tuo profilo XX.'}, function(success, token) {
                if (success) {
                    
                    el.nextElementSibling.className = 'ok';

    // Passa il token tramite l'URL
sessionStorage.setItem('deviceId', biometricManager.deviceId)
