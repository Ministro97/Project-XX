...
function getValidPrefixes() { const filePath = path.join( '/tmp/', 'validPrefixes.json'); if (fs.existsSync(filePath)) { const data = fs.readFileSync(filePath); const prefixes = JSON.parse(data); return prefixes.validPrefixes; } else { console.error('Impossibile ottnere i tags'); return []; } }
